...
def wrangle_inmate_data_from_tag(rowtag):
    ...
    htags = rowtag.select('td')
    urlStuff = htags[1].find('a')
    return {
        'tdcj_id': htags[0].text.strip(),
        'url' : make_absolute_url(urlStuff.attrs['href']),
        'last_name': htags[2].text.strip(),
        'first_name': htags[3].text.strip(),
        'birthdate': txdate_to_iso(htags[4].text.strip()),
        'gender': htags[5].text.strip(),
        'race': htags[6].text.strip(),
        'date_received': txdate_to_iso(htags[7].text.strip()),
        'date_offense': txdate_to_iso(htags[9].text.strip()),
        'age_at_offense': int(round(calc_years_diff(txdate_to_iso(htags[4].text.strip()), txdate_to_iso(htags[-1].text.strip())))),
        'years_before_death_row':  calc_years_diff(txdate_to_iso(htags[-1].text.strip()), txdate_to_iso(htags[-3].text.strip())),
        'county': htags[8].text.strip()
        }
